...
# ncu() { ncu --workspace projects/webext-*"  ;}
# ncu() { ncu --workspace projects/webext-*"  ;}

# readarray -t projects < <(ls projects | grep webext)

copy             () { cp ../utils/$1 $1                                                             ;}
copyConfigs      () { copy .eslintrc.json && copy .gitignore && copy run.sh && copy CONTRIBUTING.md ;}
_ln () { rm $1 && ln ../utils/$1 . ;}
lnConfigs () { _ln .eslintrc.json && _ln .gitignore && _ln run.sh && _ln CONTRIBUTING.md ;}

toSubmodule () {
     git submodule add https://github.com/icetbr/$1 projects/$1
     git rm --cached -r -f -- /home/ddv/dev/projects/$1
}


# for d in projects/webext*/
# do
#      # (cd "$d" && ./run.sh syncConfigs)
#      (cd "$d" && lnConfigs)
# done
